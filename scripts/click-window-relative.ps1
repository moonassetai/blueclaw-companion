...
from blueclaw_companion.window_control import click_bluestacks_relative
...
if ($Json) {
    ...
    Write-Host ("Clicked BlueStacks at relative ({0}, {1}) -> screen ({2}, {3})" -f $payload.relative_x, $payload.relative_y, $payload.screen_x, $payload.screen_y)
}
